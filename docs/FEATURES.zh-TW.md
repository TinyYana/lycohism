...
> 本文以目前工作樹的 **v0.9.34-ALPHA WIP 程式與預設 YAML** 為準；每個版本都有實際進遊戲測試，但編譯／測試通過與單機測試不等於長期多人 Paper 實機驗收。
...
- **v0.9.34 Beta onboarding pass**：新玩家首次登入導引（5 行提示）、`/lyco progress` 作主線入口說明、封印祠關鍵方塊防拆保護、文件更新。
- **封印祠保護（v0.9.34）**：控制器方塊（鑿刻深板岩）與寶箱方塊在玩家解封前受保護，無法直接拆掉繞過謎題；解封後不受限制。
- **新玩家導引（v0.9.34）**：第一次進服顯示一次 5 行提示，說明晨露採集、調律手冊、`/lyco progress` 與設施開啟手勢。
- **還沒收口的地方（Beta 前 TODO）**：三台自動化仍共用配方池；地獄廢址只有探索／配方作用，尚未成為真正的廢棄能量網；`/lyco progress` 分頁 UI 待實測後決定。
- **Spigot 相容性**：暫不處理。程式重度使用 Paper 的 Adventure/MiniMessage；等有空再說。

> 本系統仍為 ALPHA，內容會持續依實玩調整。
