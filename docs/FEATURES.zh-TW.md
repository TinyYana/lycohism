# Lycohism 玩法與特色全覽（v0.9.33-ALPHA）
...
> 本文以目前工作樹的 **v0.9.33-ALPHA WIP 程式與預設 YAML** 為準；每個版本都有實際進遊戲測試，但編譯／測試通過與單機測試不等於長期多人 Paper 實機驗收。
...
- **v0.9.33 工作樹**：封印祠、三台自動化、書房 Lv3 上限、地獄廢址與焰輝素材線都已進程式；離線 test/build 通過，每個版本也都有實際進遊戲測試，長期多人 Paper 實機仍待跑。
- **還沒收口的地方**：封印祠只鎖寶箱、沒有整座防拆；三台自動化共用配方池；地獄廢址只有探索／配方作用，尚未成為真正的廢棄能量網。
