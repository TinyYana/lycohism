...
溫室跟書房升到 Lv2 後，器物頁還會多出兩張藍圖：**育苗機**用堆肥桶當控制器，說明偏植物加工；**現象凝縮台**用講台當控制器，說明偏素材凝縮。三台目前共用一份配方池，所以真的操作時不用太糾結「這個原料到底算哪類」——放進任何一台都可能會跑。這是 ALPHA 的簡化，之後要不要分流等實玩再決定。
...
解封進度是每位玩家分開記的；同一座祠，別人解過不代表你也解過。先提醒一個 ALPHA 邊界：目前鎖的是寶箱，整座結構還沒有防拆，別拿鎬子把謎題物理解掉。
